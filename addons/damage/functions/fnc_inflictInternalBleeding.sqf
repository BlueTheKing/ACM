...
if (_woundSeverityID == 1 && (random 100) > 50) exitWith {};
...
if (_foundInjury) then {
    ...
    private _newBleedRate = (_targetWoundCount * _targetWoundBleedRate + (_bleedRate / 2)) / _newWoundCount;
    _woundsPart set [_index, [_targetWoundID, _newWoundCount, _newBleedRate]];
} else {
    // bodypart <- [WoundID, count, bleeding]
    _woundsPart pushBack [_targetWoundID, 1, (_bleedRate / 2)]; // TODO add setting
};
