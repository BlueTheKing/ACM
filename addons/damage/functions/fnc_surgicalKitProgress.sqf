 ...
 *
 * Example:
 * [[objNull, player, "head"], 5, 10, 0, false] call ace_medical_treatment_fnc_surgicalKitProgress; // ACM_damage_fnc_surgicalKitProgress;
 *
 * Public: No
 */
...
if (_totalTime - _elapsedTime > ([_patient, _bodyPart, _woundType, _useSuture] call FUNC(getStitchTime)) - _timeToStitch) exitWith {true};
...
if (ACEGVAR(medical_treatment,consumeSurgicalKit) == 2 && {_targetWoundsOnPart isNotEqualTo []}) exitWith {
    ([_medic, _patient, ["ACE_suture"]] call ACEFUNC(medical_treatment,useItem)) params ["_user"];
    !isNull _user;
};

if ((ACEGVAR(medical_treatment,consumeSurgicalKit) != 2) && _useSuture && {_targetWoundsOnPart isNotEqualTo []}) exitWith {
    ([_medic, _patient, ["ACE_suture"]] call ACEFUNC(medical_treatment,useItem)) params ["_user"];
    !isNull _user;
};
