...
_patient setVariable [QGVAR(IBCoagulation_LastClotTime), CBA_missionTime];
...
private _id = [{
    ...
    if (!(alive _patient) || ((_patient getVariable [QGVAR(IBCoagulation_LastClotTime), -1]) + 120 < CBA_missionTime)) exitWith {
...

    {
        ...
        if (_internalWoundsOnPart isEqualTo [] || [_patient, _x] call ACEFUNC(medical_treatment,hasTourniquetAppliedTo)) then {
            continue;
        };
        ...
        if (_woundIndex != -1) exitWith {
            (_internalWoundsOnPart select _woundIndex) params ["_woundType", "_woundCount", "_woundBleeding"];
                ...
                if (_woundSeverity == 1 || {_woundSeverity == 2 && (random 100 < (60 * (0.5 + (0.5 max _txaCount))))}) then {
