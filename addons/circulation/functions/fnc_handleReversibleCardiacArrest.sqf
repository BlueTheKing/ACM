...
if (_patient getVariable [QGVAR(Cardiac_RhythmState), ACM_Rhythm_Sinus] == ACM_Rhythm_PEA || _patient getVariable [QGVAR(ReversibleCardiacArrest_PFH), -1] != -1) exitWith {};
...
private _PFH = [{
    ...
    if (_patient getVariable [QGVAR(Cardiac_RhythmState), ACM_Rhythm_Sinus] != ACM_Rhythm_PEA || !_reversibleCause || !(IN_CRDC_ARRST(_patient)) || !(alive _patient) || ((_time + 360) < CBA_missionTime)) exitWith {
        _patient setVariable [QGVAR(ReversibleCardiacArrest_State), false, true];
        
        if (IN_CRDC_ARRST(_patient) && (alive _patient) && ((_time + 360) > CBA_missionTime) && _patient getVariable [QGVAR(Cardiac_RhythmState), ACM_Rhythm_Sinus] == ACM_Rhythm_PEA) then { // Reversed
