...
if (GET_BLOOD_VOLUME(_patient) < 3.4) exitWith {
...
[{
    ...
    (GET_BLOOD_VOLUME(_patient) < 3.4) || !(alive _patient) || !(IN_CRDC_ARRST(_patient))
}, {
    params ["_patient"];

    if (alive _patient && (GET_BLOOD_VOLUME(_patient) < 3.4)) then {
