...
private _fnc_chargedPFH = {
    ...
    [{
        ...
        if !([_patient, "", 1] call FUNC(hasAED)) then {
            _patient setVariable [QGVAR(AED_Charged), false, true];
...
[{
    params ["_medic", "_patient", "_manual", "_fnc_chargedPFH"];

    if (_manual) then {
        _patient setVariable [QGVAR(AED_Charged), true, true];
        [_medic, _patient] call _fnc_chargedPFH;
    } else {
        playSound3D [QPATHTO_R(sound\aed_standclear_pushtoshock.wav), _patient, false, getPosASL _patient, 15, 1, 15]; // 2.557s

        [{
            params ["_medic", "_patient", "_fnc_chargedPFH"];

            _patient setVariable [QGVAR(AED_Charged), true, true];

            [_medic, _patient] call _fnc_chargedPFH;
        }, [_medic, _patient, _fnc_chargedPFH], 2.5] call CBA_fnc_waitAndExecute;
    };
}, [_medic, _patient, _manual, _fnc_chargedPFH], 4.1] call CBA_fnc_waitAndExecute;
