...
private _healQueue = _this getVariable [QACEGVAR(medical_ai,healQueue), []];
...
if (!alive _target || {!(_target call ACEFUNC(medical_ai,isInjured))} || {
    private _treatmentEvent = (_this getVariable [QACEGVAR(medical_ai,currentTreatment), []]) param [2, ""];
    ...
    _this setVariable [QACEGVAR(medical_ai,healQueue), _healQueue];
    ...
    _this setVariable [QACEGVAR(medical_ai,nextMoveOrder), nil];
    _this setVariable [QACEGVAR(medical_ai,currentTreatment), nil];
...
if (_this distance _target > 2.5) exitWith {
    _this setVariable [QACEGVAR(medical_ai,currentTreatment), nil];
    if (CBA_missionTime >= (_this getVariable [QACEGVAR(medical_ai,nextMoveOrder), CBA_missionTime])) then {
        _this setVariable [QACEGVAR(medical_ai,nextMoveOrder), CBA_missionTime + 10];
...
[_this, _target] call ACEFUNC(medical_ai,healingLogic);
