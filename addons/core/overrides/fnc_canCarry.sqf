...
if !((_alive || _isPerson) && {_target getVariable [QACEGVAR(dragging,canCarry), false]} && {isNull objectParent _target}) exitWith {false};
...
if (_isPerson) exitWith {
    ((!_alive) && {missionNamespace getVariable [QACEGVAR(dragging,canMoveDead), true]}) ||
    {(isAwake _target) && // not ragdolled if alive
    {!(_target call ACEFUNC(common,isAwake)) ||
    {_target getVariable [QGVAR(Lying_State), false] ||
