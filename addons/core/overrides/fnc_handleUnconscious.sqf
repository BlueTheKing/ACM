...
if (_player getVariable [QACEGVAR(dragging,isDragging), false]) then {
    ...
    if (_unit == _draggedObject) then {
        ...
        [QACEGVAR(common,switchMove), [_unit, "AinjPpneMrunSnonWnonDb_still"]] call CBA_fnc_globalEvent; // Force dragging animation to avoid sliding
    };
};
...
if (_player getVariable [QACEGVAR(dragging,isCarrying), false]) then {
    ...
    if (_unit == _carriedObject) then {
        ...
        [QGVAR(cancelCarryingPrompt), [_carriedObject, _player], _carriedObject] call CBA_fnc_targetEvent;
    };
};
