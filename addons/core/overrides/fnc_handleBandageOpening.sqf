...
params ["_target", "_impact", "_part", "_injuryIndex", "_injury", "_bandage"]; // ace_medical_treatment_fnc_handleBandageOpening
...
private _plateletEffect = linearConversion [2.5, 1, (_target getVariable [QEGVAR(circulation,Platelet_Count), 3]), 0.5, 1, true];
...
if (random 1 <= _reopeningChance * _plateletEffect * ACEGVAR(medical_treatment,woundReopenChance)) then {
    ...
    [{
        ...
        if (_selClassID == _classID) then { // matching the IDs
            ...
            private _reopenChance = linearConversion [1.1, 0.1, ([_target, "TXA_IV", false] call ACEFUNC(medical_status,getMedicationCount)), 0, 1];

            if (_exist && {random 1 < _reopenChance}) then {
